...
- [背景](#背景)
  - [クマの出没増加](#クマの出没増加)
  - [秋田県ツキノワグマ等情報マップシステム「クマダス」](#秋田県ツキノワグマ等情報マップシステムクマダス)
  - [国際教養大学におけるクマ対策](#国際教養大学におけるクマ対策)
- [設定方法](#設定方法)
  - [0. 前提](#0-前提)
  - [1. Google Apps Scriptファイルの作成](#1-google-apps-scriptファイルの作成)
  - [2. スクリプトの転記](#2-スクリプトの転記)
  - [3. スクリプトプロパティの設定](#3-スクリプトプロパティの設定)
  - [4. スクリプトの実行トリガーを設定](#4-スクリプトの実行トリガーを設定)
- [本ソリューションの制約と今後への期待](#本ソリューションの制約と今後への期待)
- [免責](#免責)
...
> ▲[秋田県オープンデータカタログサイト](https://opendata.pref.akita.lg.jp/)でダウンロードした「【秋田県】ツキノワグマ等情報マップシステム『クマダス』データ（20250430時点）」[^12] から、`獣種`が`ツキノワグマ`の情報を抽出して作成[^13]。

[^12]: [【秋田県】ツキノワグマ等情報マップシステム『クマダス』データ（20250430時点）](https://ckan.pref.akita.lg.jp/dataset/050008_shizenhogoka_003)

[^13]: [AIU近辺でのクマダス情報可視化（ツキノワグマ）](https://public.tableau.com/app/profile/taro.tsukagoshi/viz/_17553189072090/AIU) （Tableau Public）

上記地図上では、AIUから周辺の出没情報のうち、半径5km以内のものを黄色の点で、半径6km以内のものを赤色の点で、それぞれ示している[^14]。半径6kmというのは、AIUを起点に、徒歩・自転車での行動圏内の目安となる秋田市御所野・河辺（和田駅周辺）・雄和中心部および秋田空港が一円に収まる範囲となっているが、先述のように、クマダスのメール配信は、任意の地点周辺5km以上の距離についてはサポートしていない。

[^14]: 緯度経度に基づく2地点間の距離は、地球を、半径6378.137km（赤道半径）の球体とみなしてHaversine Formulaを用いて算出した。
...
> [!TIP]
> `トリガーのタイプ` や `時間の間隔` は、必要に応じて変更すればいい。ここはつまり、クマダスの配信メールが新規に届いているかどうかを確認する頻度。最短は `1分おき` だが、[Google Apps Scriptの実行頻度等にも制限がある](https://developers.google.com/apps-script/guides/services/quotas)ことに加えて、1分おきではエラーが起きやすいため、推奨しない。

以上で設定は完了。

## 本ソリューションの制約と今後への期待

本ソリューションは、クマダスのメール配信の形式に依存している手法を採っている関係上、次のような制約がある:

- メール配信の文面（フォーマット）が変更された場合、スクリプトの修正が必要になる可能性が高い。
- 現行のスクリプトでは、フォーマットが変更されたことを検知する仕組みは特に導入していない。このため、Chatスペースに配信がないことは、必ずしも該当エリアでのクマ出没情報がないことを意味しない。
  - この点は、スクリプトの改良により、ある程度は改善可能。ニーズに応じて対応したい。

より確実に、任意の媒体でクマダス情報を受け取れるようにするためには、秋田県がクマダスのAPIを公開してくれることを期待したい。例えば、[秋田県オープンデータカタログサイト](https://opendata.pref.akita.lg.jp/)で提供されているクマダスのデータセット[^12]のように、定期的に更新されるJSONやCSVファイルを提供してくれるだけでも、より安定的に情報を取得できるようになる。

## 免責

- 本ソリューションで使用しているクマダスのデータは、利用者が、自身の責任において[クマダスの利用規約](https://kumadas.net/term)に同意した上で利用してください。
- 本ソリューションは、クマダスのデータを正確に取得・配信することを保証するものではありません。詳しくは[本ツールのライセンス](./LICENSE)を参照してください。
- 本ソリューションの開発の経緯や本稿の内容は開発者自身のものであり、秋田県や国際教養大学の意見を代表するものではありません。
